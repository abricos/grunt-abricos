...
Config.prototype = {
    ...
    load: function(){
        this.logger().debug('starts loading config file');

        var config = this._config;
        var configFile = path.join(config.directory, CONFIG_FILE);

        if (fs.existsSync(configFile)){
            this.logger().debug('config file is found, begin loading %s', logHelper.path(configFile));
            try {
                var configJSON = fse.readJSONFileSync(configFile);
                this._config = merge.recursive(this._config, configJSON || {});
            } catch (e) {
                this.logger().error('JSON Syntax error in %s', logHelper.path(configFile));
            }
        } else {
            this.logger().debug('config file not found %s ', logHelper.path(configFile));
        }
        delete this._logger;
    },
