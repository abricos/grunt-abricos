...
var DEFAULT_OPTIONS = {
    source: {
        dir: '<%= ^.source.dir %>/content'
    },
    build: {
        dir: '<%= ^.build.dir %>/content'
    },
    log: {
        console: {
            label: '<%= ^.log.console.label %>.content'
        }
    }
};

var ContentManager = function(owner){
    this.config = owner.config.children.create('content');
