...
me.parseModule = function(str){
    var parsed = me.parseElements('mod,mcss,mjs,ph', str, {
        named: true
    });
    ...
    var mods = {}, pi, bricks, css, js, a, modName;
    for (var i = 0; i < parsed.length; i++){
        ...
        if (pi.tag === 'mod'){
            ...
            mods[modName].phrase = mods[modName].phrase || {};
            mods[modName].phrase[a[1]] = pi.value;
        }
    }
...
me.parseGlobalVar = function(str){
    var parsed = me.parseElements('var', str, {
        named: true
    });
...
me.parseCSSFile = function(str){
    var parsed = me.parseElements('css', str, {
        removeNewLine: true
    });
    var files = [], pi;
    for (var i = 0; i < parsed.length; i++){
...
me.parseTemplateCSSFile = function(str){
    var parsed = me.parseElements('tcss', str, {
        removeNewLine: true
    });
    var files = [], pi;
    for (var i = 0; i < parsed.length; i++){
...
me.parse = function(content){
    ...
    if (a){
        ...
        ret.jsFile = me.parseJSFile(strHeader);
        ret.cssFile = me.parseCSSFile(strHeader);
        ret.templateCSSFile = me.parseTemplateCSSFile(strHeader);
        ret.module = me.parseModule(strHeader);
        ret.var = me.parseVar(strHeader);
        ret.globalVar = me.parseGlobalVar(strHeader);
