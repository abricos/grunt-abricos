...
JSComponent.prototype = {
    ...
    build: function(callback){
        ...
        stack.push(function(stackCallback){
            ...
            var destFile = path.join(buildDir, compName + '.js');
...
var JSManager = function(parentConfig, options){
    if (!parentConfig){
        throw new Error('`parentConfig` not set in JSManager');
    }
    ...
    var configId = [parentConfig.id, "js"].join(".");
    var config = Config.instance(configId, options);
    this.config = config;

    var logger = this.logger();

    var files = glob.sync(path.join(config.get('source.dir'), '*.js')),
        list = this.list;
