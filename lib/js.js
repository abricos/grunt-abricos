...
var JSManager = function(module){
    ...
    logger.debug('souce dir `%s`', path.relative(process.cwd(), srcDir));
    logger.debug('build dir `%s`', path.relative(process.cwd(), buildDir));
    logger.debug('temp dir `%s`', path.relative(process.cwd(), tempDir));
    ...
    this.logger().debug('initialized, files='+files.length);
};
JSManager.prototype = {
    logger: function(){
